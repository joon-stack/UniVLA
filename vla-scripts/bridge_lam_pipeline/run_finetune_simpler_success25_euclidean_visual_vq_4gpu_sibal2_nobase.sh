...
SIMPLER_DATA_ROOT="/NHNHOME/WORKSPACE/0526040036_A/user/01/youngjoonjeong/data/simpler_mvplam_noeval_nested_success25_rlds"
...
FINETUNE_RUN_ID_NOTE="${FINETUNE_RUN_ID_NOTE:-simpler_success25_euclidean_visual_vq_vla50k_4gpu_b32_acc1_sibal2_workers2_trainloop_lam}"
FINETUNE_MASTER_PORT="${FINETUNE_MASTER_PORT:-28761}"
FINETUNE_RUN_ROOT="${FINETUNE_RUN_ROOT:-${ROOT}/outputs/finetune_simpler_success25_euclidean_visual_vq_vla50k_sibal2_workers2_4gpu}"
FINETUNE_ADAPTER_TMP_DIR="${FINETUNE_ADAPTER_TMP_DIR:-${ROOT}/outputs/finetune_simpler_success25_euclidean_visual_vq_vla50k_sibal2_workers2_4gpu_adapter_tmp}"
FINETUNE_SHUFFLE_BUFFER_SIZE="${FINETUNE_SHUFFLE_BUFFER_SIZE:-512}"
FINETUNE_IMAGE_AUG="${FINETUNE_IMAGE_AUG:-true}"
LOG_ROOT="${LOG_ROOT:-${ROOT}/outputs/bridge_pipeline_logs/euclidean_visual_vq_simpler_success25_sibal2_workers2_4gpu}"
...
env \
  ...
  HF_UPLOAD_REPO_ID="${HF_UPLOAD_REPO_ID:-joon-stack/univla-simpler-success25-euclidean-visual-vq-vla50k-nobase-step10000}" \
  ...
  HF_UPLOAD_NOTES="${HF_UPLOAD_NOTES:-UniVLA Simpler success25 euclidean visual VQ nobase finetune.}" \
